...
class LeNetCIFARKFAC(nn.Module):
    ...
    def initKFACStorage( self, batchSize ): 
        global Zis
        global dxa
        ...
        Zis.append( None )
        Zis.append( torch.zeros( (batchSize, 6, 14, 14) ).type(torch.cuda.DoubleTensor) )
        Zis.append( torch.zeros( (batchSize, 16, 5, 5) ).type(torch.cuda.DoubleTensor ) )
        Zis.append( torch.zeros( (batchSize, 120 )).type(torch.cuda.DoubleTensor ) )
        Zis.append( torch.zeros( (batchSize, 84 )).type(torch.cuda.DoubleTensor ) )

        #Fill the dxA's now...
        dxa.append( torch.zeros( (batchSize, 6, 28, 28) ).type(torch.cuda.DoubleTensor) )
        dxa.append( torch.zeros( (batchSize, 16, 10, 10 )).type(torch.cuda.DoubleTensor) )
        dxa.append( torch.zeros( (batchSize, 120)).type(torch.cuda.DoubleTensor ) )
        dxa.append( torch.zeros( (batchSize, 84)).type(torch.cuda.DoubleTensor ) )
        dxa.append( torch.zeros( (batchSize, 10)).type(torch.cuda.DoubleTensor ) )


    def forward(self, x): 
        if self.activation == 'softplus': 
            out = self.conv1( x ) 
            #out = self.conv1_bn(  F.softplus(out) )
            out = F.softplus(out)
            ...
            out = self.conv2( out )
            #out = self.conv2_bn( F.softplus(out) )
            out = F.softplus(out)
            out = F.avg_pool2d(out, 2)
            if (self.KFACEnabled): 
            ...
            out = self.fc1( out )
            #out = self.fc1_bn( F.softplus(out) )
            out =  F.softplus(out) 
            if( self.KFACEnabled ): 
                Zis[ 3 ].copy_( out.data )

            out = self.fc2( out )
            #out = self.fc2_bn( F.softplus(out) )
            out = F.softplus(out)
            if( self.KFACEnabled ): 
    ...
    def computeTempsForKFAC( self, X ): 
        ...
        x_var = Variable( X )
        out = self (x_var)
        loss = self.lossFunction( out, torch.max( out, 1 )[1] )
        g = self._computeGradient( loss, False )
        x_var.volatile = True



    def computeGradientIter( self, X, Y ): 
        ...
        lossreg = 0.5 * self.regularization * torch.dot( W, W )
        gradreg = self.regularization * W

        return (loss.cpu().data[0] / X.shape[0] + lossreg ), (g / X.shape[0]).data.add_(gradreg)
        #return (loss.cpu().data[0] / X.shape[0] + lossreg ), (g.data / X.shape[0] + gradreg), accu.data[0]
        #return g
        
    def computeHv( self, sampleX, sampleY, vec ): 

        X_sample_var = Variable( sampleX, requires_grad=True )
        Y_sample_var = Variable( sampleY )

        self.zero_grad ()
        loss, pred = self.evalModelChunk( X_sample_var, Y_sample_var )
        gradient = self._computeGradient( loss, True )
        #import pdb;pdb.set_trace();
        self.zero_grad ()
        hv = self._computeGradient( (gradient * vec).sum (), False )

        X_sample_var.volatile = True
        Y_sample_var.volatile = True

        return hv / sampleX.shape[0] + self.regularization *  vec 

    def computeHv2( self, sampleX, sampleY, vec ): 
    ...
    def computeKFACHv( self, X, vec, ignoreTemps ): 
        global Zis
        global dxa
    ...
    def computeInitialKFACTerms( self, X ): 
        self.kfac.updateZAndD( Zis, dxa )
        self.kfac.prepMatVec( X, True )
    ...
    def updatePackedWeights( self, vec ): 
        idx = 0
        for W in self.parameters ():
    ...
    def getWeights( self ): 
        return torch.cat([ w.contiguous().view( -1 ).data for w in self.parameters () ])
