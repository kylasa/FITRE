...
class SubsampledTRCGKFACFULL: 
	...
	def fullPass( self, train ): 
		...
		for l in train: 
			...
			vHv = torch.dot( self.network.computeHv( X_sample_var , Y_sample_var , x_kfac ), x_kfac )
			...
			if ( vHv.data[ 0 ] < 0 ): 
				...
				gv = torch.dot( x_kfac, grad ).data.cpu ()
				step = gv / (vHv.data.cpu () + 1e-6)

				#import pdb;pdb.set_trace();

				step = - torch.min( step, (self.params.delta / (torch.norm(x_kfac).cpu ().data + 1e-16)) )
				kfac_step = step

			m_kfac = step * torch.dot( x_kfac, grad ).data.cpu () + 0.5 * step * step * vHv.data.cpu ()

			#import pdb;pdb.set_trace();
			t = self.network.computeHv( X_sample_var, Y_sample_var, grad )
			vHv = torch.dot( t, grad )
			if (vHv.data[ 0 ] < 0 ): 
				step = -(self.params.delta) / torch.norm( grad ).data
				grad_step = step
			else: 
				gv = torch.dot( grad, grad ).data.cpu ()
				step = gv / (vHv.data.cpu () + 1e-6)
				step = - torch.min( step, self.params.delta / (torch.norm(x_kfac).data + 1e-16) )
				grad_step = step

			#import pdb;pdb.set_trace();

			m_g_kfac = step * torch.dot( grad, grad ).data.cpu () + 0.5 * step * step * vHv.data.cpu ()

			self.network.zero_grad ()
			if (m_kfac.numpy () <= m_g_kfac.numpy ()): 
				...
				#import pdb;pdb.set_trace();
				self.network.updateWeights(  x_kfac.data.mul_( kfac_step.numpy()[0] ) )

				#for X, Y in self.train: 
				self.network.zero_grad ()
				new_ll, new_accu = self.network.evalModel( X_sample_var, Y_sample_var )
				self.stats.no_props += len( self.train )
				rho = (tr_ll - new_ll) / (-m_kfac.numpy ()[0])

				if (rho.data[0] > 0.75) : #1e-4 
					self.x = self.x.add( x_kfac.data.mul_( kfac_step.numpy()[0] ) )
				...
				#for X, Y in self.train: 
				tr_ll, tr_accu = self.network.evalModel( X_sample_var, Y_sample_var )
				self.network.updateWeights( grad.data.mul_( grad_step.numpy ()[0] ) )

				#for X, Y in self.train: 
				self.network.zero_grad ()
				new_ll, new_accu = self.network.evalModel( X_sample_var, Y_sample_var )
				self.stats.no_props += len( self.train )
				rho = (tr_ll - new_ll) / (-m_g_kfac.numpy ()[0])

				if (rho.data[0] > 0.75) : #1e-4 
					self.x = self.x.add( grad.data.mul_( grad_step.numpy()[0] ) )
					self.params.delta = min( self.params.max_delta, self.params.gamma1 * self.params.delta ) # 2
				elif( rho.data[0] >= 0.25 ): 
					self.x = self.x.add( grad.data.mul_( grad_step.numpy()[0] ) )
				else: 
