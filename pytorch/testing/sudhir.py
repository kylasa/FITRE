...
c1 = torch.nn.Conv2d (1, 1, 1)
c1.weight.data.fill_(np.float64(0.1))
c1.bias.data.fill_(np.float64(0.1))
c1.double ()


#temp = readWeights.readMatrix( '../../cuda_weights.txt', [ [1, 1, 1, 1], [1] ] )
#cw1 = np.reshape( temp[0], (1, 1, 3, 3), order='F' )

#c1.weight.data.copy_ (torch.from_numpy( temp[0]  ).type( torch.DoubleTensor ))
#c1.bias.data.copy_(torch.from_numpy( np.asarray([0])).type( torch.DoubleTensor ))

c1.weight.data.copy_ (torch.from_numpy( np.asarray ([1])  ).type( torch.DoubleTensor ))
c1.bias.data.copy_(torch.from_numpy( np.asarray([0])).type( torch.DoubleTensor ))
...
#temp = readWeights.readMatrix( '../../cuda_weights2.txt', [ [1, 1, 1, 1], [1] ] )
#cw1 = np.ravel( np.reshape( temp[0],(1, 1, 3, 3)  ))
#cw1 = np.ravel(temp[0] )
#cb1 = temp[1]
...
#cw1 = np.asarray( [ 0, 0, 0, 0, 0, 0, 0, 0, 0 ] )
#cb1 = np.asarray( [ 0.1 ] )
