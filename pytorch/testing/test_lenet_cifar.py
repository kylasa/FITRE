__author__ = 'Sudhir Kylasa'

import os
import datetime
import sys
import numpy as np
import math

import torch
from torch import nn
from torch.autograd import Variable

from dataset_loader import TensorDataset
from lenet_cifar import LeNetCIFAR

#torch.cuda.set_device( 1 )

#Read the dataset here. 
data_path = '/scratch/skylasa/solvers/cnn-data/cifar-10-batches-py/'
train = TensorDataset( csv_path=data_path, fileFilter='data_batch_1' )
train_data, train_labels = train.getDataset ()

train_data = torch.from_numpy( train_data ).type(torch.DoubleTensor)
train_labels = torch.from_numpy( train_labels ).type(torch.LongTensor)

model = LeNetCIFAR( num_classes = 10, activation='softplus')
model.double ()
#model.cuda ()
model.initZeroWeights ()
model.initWeights( 0.1 ) 

criterion = nn.CrossEntropyLoss (size_average=False)
model.setLossFunction( criterion )

train_data[ 0, :, :, : ] = 1; 

print( train_data[ 0, :, :, : ].shape )
print( torch.unsqueeze( train_data[ 0, :, :, : ], 0 ).shape )

loss = model.evalModel( torch.unsqueeze( train_data[ 0, :, :, : ], 0 ), torch.from_numpy( np.asarray( [ train_labels[ 0 ] ] )  ) )
print (loss)

#Gradient
grad = model.computeGradientIter( torch.unsqueeze( train_data[ 0, :, :, : ], 0 ), torch.from_numpy( np.asarray( [ train_labels[ 9 ] ] )  ), 0 )
print( 'Gradient-----' )
print( len(grad) )

for i in grad: 
	print( i.data.cpu().numpy().flatten()[1:10] )

#HessianVec
#gv = torch.cat( [ g.contiguous ().view( -1 ) for g in grad if g is not None ] )

