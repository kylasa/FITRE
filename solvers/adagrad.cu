...
#ifdef STATS
void ADAGRADOutputModelParams( FILE *out, ADAGRAD_PARAMS *mParams )
{
   fprintf( out, "\n"); 
	fprintf( out, " ********** ADAGRAD REPORT ********** \n");
   fprintf( out, "\n"); 
   fprintf( out, "\n"); 
...
void ADAGRADCleanup (ADAGRAD_OUT_PARAMS *out )
{  
   if (out->out) fclose ( out->out );
}

void ADAGRADInitOutParams( ADAGRAD_OUT_PARAMS *adagradOut)
{
		adagradOut->iteration = 0;
		adagradOut->trainLL = 0; 
		adagradOut->trainModelErr = 0;  
		adagradOut->testLL = 0; 
		adagradOut->testModelErr = 0; 
		adagradOut->normGrad = 0;
		adagradOut->numProps = 0; 
		adagradOut->iter_time = 0; 
		adagradOut->total_time = 0; 
}

void ADAGRADInitOutputFile(ADAGRAD_OUT_PARAMS *out, ADAGRAD_PARAMS *mParams)
{
   if ( (out->out = fopen("ADAGRAD_OUTPUT.txt", "w")) == NULL ) {
      fprintf( stderr, "Error opening output write file....... !\n" );
      exit( -1 );
   }

   //Header line
   ADAGRADOutputModelParams( out->out, mParams );
   ...
   ADAGRADInitOutParams( out );
}

void ADAGRADWriteOutLine( ADAGRAD_OUT_PARAMS *out )
{
					...
					SCRATCH_AREA *scratch, ADAGRAD_PARAMS *params)
{
   ...
   int n = data->trainSizeX;
	int datasetLoops = (n / params->sampleSize) + 1; 
   int sampleSize = params->sampleSize;
	real iter_start, iter_end; 
	real iter_running; 
	ADAGRAD_OUT_PARAMS adagradOut; 
	real normGrad;
	real alpha; 
...
#ifdef STATS
	ADAGRADInitOutputFile( &adagradOut, params ); 	
#endif
	...
	for (int iter = 0; iter < params->maxEpochs; iter ++){
		...
		for (int j = 0; j < datasetLoops; j ++) {
      	...
      	cublasCheckError( cublasDaxpy( cublasHandle, model->pSize, &alpha, data->weights, 1, gradient, 1 ) );
			...
			// update the step Adam's algorithm
			// cache = gradient ** 2
			...
			kerElemSqr <<<numBlocks, BLOCK_SIZE >>> 
				(gradient, model->pSize, cache ); 
      ...
      *trainLogLikelihood += 0.5 * params->lambda * (*nextPageLckPtr);
		...
		//stats here. 
		adagradOut.iteration = iter;
		adagradOut.trainLL = *trainLogLikelihood; 
		adagradOut.trainModelErr = *trainModelError; 
		adagradOut.testLL = *testLogLikelihood; 
		adagradOut.testModelErr = *testModelError; 
		adagradOut.normGrad = normGrad;
		adagradOut.numProps = numProps; 
		adagradOut.iter_time = iter_end; 
		adagradOut.total_time = iter_running; 

		ADAGRADWriteOutLine (&adagradOut); 
#endif
	}
}
